...
void Model::loadImage(const string& nombreArchivoJPG) {
  ...
  // unsigned char * texels = imagen->leerPixels(); // puntero texels (unsigned
  // char *)
}

void Model::setVertices(vector<_vertex3f> vertex) { _vertices = vertex; }

void Model::setFaces(vector<_vertex3i> faces) { _caras = faces; }
...
void Model::draw(Model::DrawMode mode) {
  ...
  if (imagen != NULL) {
    // Carga la imagen
    glTexImage2D(GL_TEXTURE_2D, 0, 3, imagen->tamX(), imagen->tamY(), 0, GL_RGB,
                 GL_UNSIGNED_BYTE, imagen->leerPixels());
  ...
  for (int i = 0; i < _caras.size(); i++) {
    ...
    if (_normales_vertices.size() > 0)
      glNormal3fv((GLfloat*)&_normales_vertices[Vertex_1]);

    glVertex3fv((GLfloat*)&_vertices[Vertex_1]);

    if (_texturas.size() > 0)
      glTexCoord2f(_texturas[Vertex_2].x, _texturas[Vertex_2].y);

    if (_normales_vertices.size() > 0)
      glNormal3fv((GLfloat*)&_normales_vertices[Vertex_2]);

    glVertex3fv((GLfloat*)&_vertices[Vertex_2]);

    if (_texturas.size() > 0)
      glTexCoord2f(_texturas[Vertex_3].x, _texturas[Vertex_3].y);

    if (_normales_vertices.size() > 0)
      glNormal3fv((GLfloat*)&_normales_vertices[Vertex_3]);

    glVertex3fv((GLfloat*)&_vertices[Vertex_3]);
  }
