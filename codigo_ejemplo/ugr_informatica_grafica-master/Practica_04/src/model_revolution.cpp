...
void Model_Revolution::process_key(unsigned char Tecla) {
  ...
  if (Tecla == 'n') varDrawNormales = !varDrawNormales;
}
...
void Model_Revolution::draw(DrawMode mode) {
  ...
  // las pintamos
  if (varDrawNormales) drawNormales();
...
void Model_Revolution::generarPorPerfil(vector<_vertex3f> perfil, int steps) {
  ...
  if (vertices.front().x) {
    // Agregamos el punto central, aunque no es necesario porque solo pintamos
    // las caras
  ...
  if (vertices[perfil.size() - 1].x) {
    ...
    for (int step = 0; step < steps; step++) {
      ...
      caras.push_back(_vertex3i(
          vertices.size() - 1, perfilactual,
          siguientePerfil));  // Hay que ponerlos al revés que en la otra tapa
                              // para que calcule bien las normales
    }
  }
...
//***************************************************************************************
// Funcion para generar y almacenar los coord. de textura (malla generada por
// revolucion)
//***************************************************************************************
