...
void _revolucion::nuevoPerfil(const vector<_vertex3f>& ver) {
  Perfil.erase(Perfil.begin(), Perfil.end());
  Perfil = ver;
}

void _revolucion::limpiar() {
  vertices.erase(vertices.begin(), vertices.end());
  caras.erase(caras.begin(), caras.end());
}
...
void _revolucion::generarPerfil(bool tapaArriba, bool tapaAbajo, char eje) {
  ...
  for (int step = 0; step < steps; step++) {
    ...
    for (unsigned int k = 0; k < Perfil.size(); k++)
      if (eje == 'Y') {
        vertice_aux = rotateY(Perfil[k], angle);
        if (par) {
          vertice_aux.x = vertice_aux.x * 0.5;
          // vertice_aux.y = vertice_aux.y * 0.5;
          vertice_aux.z = vertice_aux.z * 0.5;
        }
        par = !par;
        siguientePerfil[k] = vertice_aux;
      } else if (eje == 'X')
...
void _jerarquico::draw(_modo modo) {
  cubo1.draw(modo, 0.8, 0.8, 0.8, 0.9, 0.9, 0.9, 2);
  glPushMatrix();
  glTranslatef(0.375, 0.375, 0.375);
  glRotatef(rotacion_cubo, 0, 1, 0);
  glTranslatef(-0.375, -0.375, -0.375);
  draw_cubos(modo);
  glPushMatrix();
  glTranslatef(0.75, 0.75, 0.75);
  glRotatef(rotacion_antena, 0, 0.75, 0);
  glTranslatef(-0.75, -0.75, -0.75);
  draw_cilindros(modo);
  glPopMatrix();
  glPopMatrix();
}
void _jerarquico::draw_cubos(_modo modo) {
  glPushMatrix();
  glScalef(0.5, 0.5, 0.5);
  glTranslatef(1.5, 3, 1.5);
  cubo2.draw(modo, 0.8, 0.8, 0.8, 0.9, 0.9, 0.9, 2);
  glPopMatrix();
}

void _jerarquico::draw_cilindros(_modo modo) {
  tubo.generarPerfil(false, false);
  glPushMatrix();
  glScalef(0.1, 2.0, 0.1);
  glTranslatef(7.5, 1.0, 7.5);
  tubo.draw(modo, 0.8, 0.8, 0.8, 0.9, 0.9, 0.9, 2);
  glPopMatrix();

  antena.generarPerfil();
  glPushMatrix();
  glScalef(0.1, 0.1, 1.0);
  glTranslatef(7.5, 27, 1.0);
