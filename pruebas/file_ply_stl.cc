...
//
// * 'nombreArchivo' no debe incluir la extensión .ply (se le añade antes de
// abrirlo)
...
void read(const char *nombreArchivo, vector<float> &vertices,
          vector<int> &faces) {
  // modelos ply en:
  ...
  unsigned state = 0;  // 0 antes de leer 'element vertex', 1 antes de leer
                       // 'element face', 2 después
  bool inHeader = true;
  ...
  if (token != "ply") error("el archivo de entrada no comienza con 'ply'.");
  ...
  while (inHeader) {
    if (src.eof()) error("fin de archivo no esperado en la cabecera");
    ...
    if (token == "end_header") {
      if (state != 2)
        error(
            "no se encuentran elementos 'element vertex' ni 'element face' en "
            "la cabecera");
      src.getline(buffer, bufferCapacity);
      inHeader = false;
      ...
      if (token != "ascii") {
        string msg = string(
                         "no se puede leer archivo en formato no 'ascii', el "
                         "formato es: '") +
                     token + "'";
        error(msg.c_str());
      }
      ...
      src >> token;
      if (token == "vertex") {
        if (state != 0) error("'element vertex' va después de 'element face'");
        src >> nVertexs;
  ...
  for (unsigned long long iv = 0; iv < nVertexs; iv++) {
    if (src.eof()) error("fin de archivo no esperado en la lista de vértices");
  ...
  for (unsigned long long ifa = 0; ifa < nFaces; ifa++) {
    if (src.eof()) error("fin de archivo no esperado en la lista de caras");
    ...
    if (nv != 3)
      error(
          "se ha encontrado una cara con un número de vértices distinto de 3.");
